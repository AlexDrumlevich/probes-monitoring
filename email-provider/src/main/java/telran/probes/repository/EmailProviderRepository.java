...
@Slf4j
@Repository
public class EmailProviderRepository {
	...
	String idFieldName;
	@Value("${sensor.collection.emailsFieldName}")
	String emailsFieldName;

	public Optional<SensorEmailsDto> findSensorById(long id) {
		
		Aggregation aggregation = Aggregation.newAggregation(
				Aggregation.match(Criteria.where(idFieldName).is(id)),
				Aggregation.project(idFieldName, emailsFieldName)
		);
		
		
		List<Document> resDocuments = mongoTemplate.aggregate(aggregation, collectionName, Document.class).getMappedResults();
		
		SensorEmailsDto resSensorRangeDto = null;
		if(resDocuments.size() == 1) {
			Document resDocument = resDocuments.get(0);
			resSensorRangeDto = new SensorEmailsDto(
					resDocument.getLong(idFieldName),
					resDocument.getList(emailsFieldName, String.class)
					);
